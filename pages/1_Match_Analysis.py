...
PROJECT_ROOT = Path(__file__).parent.parent # Goes up one level from 'pages' to the root
...
if not selected_match_data:
    ...
    with tabs[1]:
        ...
        with home_perf_col1:
            ...
            win_cols_h[1].metric(label="Clean Sheet %", value=f"{round(clean_sheet_h)}%",delta=f"{round(l5homeCS_delta)}% league avg ({round(l5HomeLeagueCleanSheet*100)}%)") #nleague average clean sheet
        ...
        with away_perf_col2:
            ...
            win_cols_a[0].metric("GG",f"{int(Last5_AwayBothTeamsToScore)}%",f"{away_btts_delta}% league avg ({round(l5_league_avg_btts)}%)")
            win_cols_a[1].metric(label="Clean Sheet %", value=f"{round(clean_sheet_a)}%",delta=f"{round(l5AwayCS_delta)}% league avg ({round(l5AwayLeagueCleanSheet*100)}%)")
